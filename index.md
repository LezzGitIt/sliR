...
for individual $`i`$ with appendage length $`A_i`$ and body mass
$`M_i`$, an arbitrary reference mass $`M_0`$, and an allometric scaling
exponent $`b`$. The allometric scaling exponent is calculated using
standardized major axis (SMA) regression, as recommended by Warton et
al. ([2012](#ref-wartonSmatr3Package2012)).
...
under isometric scaling ($`b = 1/3`$, so linear dimensions scale with
...
appends a column and returns the rows in their original order. The
function can be piped, creating several indices that sit side by side
for comparison.
...
constant. The SLI maintains the units of the appendage $`A`$ and
projects it along the line implied by the scaling coefficient to the
reference mass $`M_0`$. The ranking of individuals will never change by
changing $`M_0`$. $`M_0`$ defaults to the mean of `Mass`.
...
[`calc_sli()`](https://LezzGitIt.github.io/sliR/reference/calc_sli.md)
calls this for you. Individuals whose group is `NA` or unknown-coded
...
draws from a bivariate log-normal specified by its allometry rather than
by raw standard deviations. That allometry has only **two degrees of
freedom**: any two of the correlation `r_app_mass` and the three slopes
`b_ols`, `b_sma`, `b_avg` determine the other two. Supply two, or none
to accept the isometric default `b_sma = 1/3, r_app_mass = 0.3`.

The three slopes are not interchangeable. `b_ols` is the slope of
$`E[\log A \mid \log M]`$, and so the *generative* slope an
[`lm()`](https://rdrr.io/r/stats/lm.html) recovers. `b_sma` is
$`\sigma_A / \sigma_M`$, the slope an SMA fit returns and the exponent
[`calc_sli()`](https://LezzGitIt.github.io/sliR/reference/calc_sli.md)
assumes. Note that there is no clear consensus regarding which
line-fitting technique is best for allometric scaling and related
applications. Thus, `b_avg` chooses the midpoint between `b_ols` and
`b_sma`, which no estimator returns; it is a convention for splitting
the difference.
[`implied_allometry()`](https://LezzGitIt.github.io/sliR/reference/implied_allometry.md)
shows what any parameter set implies:
...
emit latitudes beyond $`\pm 90`$ — and leaves the extremes of the
gradient nearly unsampled.
...
positive appendage–mass correlation, and
[`sim_allometric()`](https://LezzGitIt.github.io/sliR/reference/sim_allometric.md)
...
is the simpler raw-scale counterpart, with no log transform and no
allometric target — useful for building intuition about how a fitted
slope responds to correlation and to error on one trait but not the
other.
[`build_cov_mat()`](https://LezzGitIt.github.io/sliR/reference/build_cov_mat.md)
exposes the covariance matrix that
[`sim_allometric()`](https://LezzGitIt.github.io/sliR/reference/sim_allometric.md)
consumes.
...
| [`implied_allometry()`](https://LezzGitIt.github.io/sliR/reference/implied_allometry.md) | What a set of allometric parameters implies for the remaining parameters |
| [`sim_correlated()`](https://LezzGitIt.github.io/sliR/reference/sim_correlated.md) | Raw-scale correlated appendage and mass |
| [`sim_grid()`](https://LezzGitIt.github.io/sliR/reference/sim_grid.md) | Map [`sim_allometric()`](https://LezzGitIt.github.io/sliR/reference/sim_allometric.md) over a grid of parameter combinations |
| [`build_cov_mat()`](https://LezzGitIt.github.io/sliR/reference/build_cov_mat.md) | Log-scale covariance matrix behind [`sim_allometric()`](https://LezzGitIt.github.io/sliR/reference/sim_allometric.md) |
